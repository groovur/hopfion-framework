...
def Gamma(t, q=3):
    """T(2,q) torus-knot curve. q=3 (default) is the trefoil; any odd q
    with gcd(2,q)=1 gives a genuine single-component (2,q) torus knot
    over the same parameter range t in [0,2*pi)."""
    return np.array([(R0+r0*np.cos(q*t))*np.cos(2*t),
                      (R0+r0*np.cos(q*t))*np.sin(2*t),
                      r0*np.sin(q*t)])

def Gamma_prime(t, q=3, h=1e-6):
    return (Gamma(t+h, q)-Gamma(t-h, q))/(2*h)

def build_raw_bishop_frame(NT=30000, q=3):
    t_arr = np.linspace(0, 2*np.pi, NT, endpoint=False)
    tangents = np.array([Gamma_prime(t, q) for t in t_arr])
...
def build_compensated_frame_arclength(NT=30000, q=3):
    ...
    t_arr, T, N1_raw, N2_raw = build_raw_bishop_frame(NT=NT, q=q)

    # cumulative arc length
    pts = np.array([Gamma(t, q) for t in t_arr])
