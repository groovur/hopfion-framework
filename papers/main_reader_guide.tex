...
lepton Bell tests, and the thawing dark-energy equation of state). The reader who expects a contradiction
...
\paragraph{A recurring constant.} A single modular quantity threads several
of these results: the WZW virial value $V^* = S_{0,1/2}/S_{0,0} = \vp$,
proved by three-fermion Witten bosonization (Paper~III~\cite{Paper3}),
reappears as the fine-structure constant $\aem^{-1} = k|2I|/V^{*2}
= 360/\vp^2$, as the cosmological-constant normalisation
$c = \vp^3 + V^{*-2Q_H}$ (Paper~VII~\cite{Paper7}), and as the leading
solar mixing angle $\tan\theta_{12}^{(0)} = 1/V^*$
(Paper~XVII~\cite{Paper17}); the vacuum Born weight $p_0 = 1/V^{*2}$ is
common to all three.  What looks in each sector like an independent
appearance of the golden ratio is at root the same proved quantum
dimension $V^* = d_{1/2}$.
...
Corollary~6.12).
The Vainshtein radius scales as
$r_V \propto (G_N M_\odot / m_\xi^2)^{1/3}$
with $m_\xi \sim H_0$ the physical (Einstein-frame) condensate mass.
Its golden-ratio scaling in $\vp$ is a Jordan-frame quantity, not a
frame-invariant ``topological invariant'', so the Cassini resolution is
secured frame-robustly by the \emph{chameleon} channel rather than by that
scaling.
...
is screened at cosmological densities by the suppression $\Lambda_0 e^{-\vp^7/\beta}$. A canonical leading normalisation $c=\varphi^3$ gives $\sim10^{-118}M_{\mathrm{Pl}}^4$, refined by an anyonic $p_0^2$ correction (the squared vacuum Born weight $p_0=1/\varphi^2$) to $\approx10^{-122}M_{\mathrm{Pl}}^4$, matching the observed $\Lambda_{\mathrm{obs}}$. The relevant attractor density $\rho_\infty = \vp/\beta$ is a fixed point of the
condensate equation of motion; the screening is not a fine-tuning but
a saddle-point condition. The leading $c=\varphi^3$ is the kinetic canonical normalisation; the $p_0^2$ refinement is $p_0^{Q_H}$ with $Q_H=2$ the condensate's Hopf charge --- one Chern--Simons monodromy insertion ($p_0=1/V^{*2}=1/\varphi^2$, the same $S$-matrix scalar that carries $\aem^{-1}=k|2I|\,p_0$) per unit of Hopf linking, computed as a Wilson loop in the quantised background (Paper~VII~\cite{Paper7}). The one topological input --- that the self-energy loop's linking equals the Hopf charge --- is the definition of the Hopf invariant, so the derivation stands at the same level as the leading $\aem^{-1}$ cascade.

\textbf{Dark matter.}
Dark matter is the invisible Frank elastic strain of the condensate
\emph{director} (its orientation field $\hat{n}=\hat{e}_r$), not a new
particle and not clustering of the scalar magnitude. The directional
suppression $S_{\mathrm{eff}}=\sin^4\theta$ is a semi-Dirac dispersion
anisotropy; its orientation mode is cold ($c_{\mathrm{dir}}\to0$,
pressureless), splay-unstable, and self-organises into a disclination
that gives $M_{\mathrm{enc}}\propto r$ and flat rotation curves with no
fitted halo, tracking the baryons and the radial-acceleration relation
with a MOND-scale acceleration $a_0\approx cH_0/(2\pi)$ set by the same
condensate. Being pressureless and comoving with the baryons, it adds
no acoustic feature and shifts no acoustic scale; recovery of the
standard $r_s\approx147\,\mathrm{Mpc}$ is contingent on whether the strain
supplies a cold-matter component at recombination, which is not yet fixed.
(The relativistic sound speed $c_s=1/\vp$ belongs to the
dark-\emph{energy} perturbations, not to dark matter.)
...
The paper is explicit about the status of $Z_{\mathrm{eff}}(\mathrm{O})=2$:
it is a re-parametrisation of the measured oxygen ionisation energy through
the Slater--Condon angular algebra, not an independent prediction of the
value (the screening that returns $Z_{\mathrm{eff}}=2$ is fixed by the
measured energy). The residual $O(\alpha)$ excess is numerically close to
$3\alpha/8\pi$, but that form is excluded on scaling grounds; it is the
ordinary $2p^4$ correlation energy, not a condensate correction.
...
the invisible elastic strain of its director field, not through a new particle
and not through any electromagnetically coupled state. It therefore predicts
\emph{null results} in all dark-matter direct-detection experiments, consistent
...
\textbf{The dark-matter strain magnitude.} The director-strain mechanism gives
flat rotation curves and a MOND-scale $a_0\approx cH_0/(2\pi)$ of the right
order, but the exact coefficient (the cutoff-to-crossover ratio, of order
$\vp^6$) and whether the strain also supplies the recombination-era matter
budget are set by the condensate cutoff and not yet fixed. The sector is
pressureless and comoving with the baryons, so it shifts no acoustic scale and
makes no BAO-shift prediction.
...
\item Dark matter detected as a new particle (direct detection or collider); the framework predicts strictly null.
...
\end{enumerate}
Item 1 will be resolved within the next two to three years by DESI DR2.
...
DESI thawing dark-energy prediction (awaiting DR2), the dark-matter strain
magnitude (cutoff-set), and the free-electron mass anisotropy prediction
($\Delta m/m_e = 1/\vp^8$, untested).
