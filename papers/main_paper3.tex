...
\begin{enumerate}[noitemsep]
  \item The first step ($\Delta_1 = 12.824$, $96.5\%$ of the total
    ...
    physical origin --- e.g.\ in the condensate's own vacuum topology and
    relaxation dynamics, Remark~\ref{P3:rem:delta1_candidate} --- is a
    separate, open question (Open Problem~4): $\Delta_1$ being algebraic
    while candidate physical mechanisms of this kind are generically
    transcendental means any such connection can be numerically close
    but not exact.
...
\begin{remark}[Candidate physical origin of $\Delta_1$: vacuum topology plus relaxation --- unverified]
\label{P3:rem:delta1_candidate}
A numerically precise (but not yet derived) candidate decomposition of
$\Delta_1$ has been found:
\begin{equation}
  \Delta_1 \;\approx\; 4\pi \;+\; \left(\frac{15}{8}-\vp\right)
  \;=\; 12.823\,337\ldots,
  \label{P3:eq:delta1_candidate}
\end{equation}
compared to $\Delta_1 = 12.824\,120\ldots$, a $0.0061\%$ match. Both terms
are already-established quantities, not fitted: $4\pi$ is the same
Chern--Simons action unit that generates $m_e/\Lcond$ via a $\Delta Q=1$
topological transition (Theorem 6.11 of Paper~IV~\cite{Paper4})%\ref{P4:thm:second_spoke}
; $15/8-\vp$ is
the exact distance the condensate's own virial function travels during
its physical relaxation, from the bare thin-torus value $V(0)=15/8$ to
the self-consistent fixed point $V(\bst)=\vp$ (Lemma 4.3 of Paper~IV~\cite{Paper4}%\ref{P4:lem:virial_factor}
;
Theorem~\ref{P3:thm:V_exact_all_R0} here). Using the $R_0=3$-corrected
virial $V(0;R_0)=\tfrac{15}{8}(1+\tfrac{1}{2R_0^2})$ in place of the
thin-torus value \emph{worsens} the match to $0.81\%$; since $\Delta_1$
itself has no $R_0$-dependence (both $\vp^6/\sin^4(\pi/5)$ and $360/\vp^2$
are $R_0$-independent), this is consistent with --- not evidence against
--- the candidate, rather than a discriminating failure.

\textbf{Proposed physical picture (unverified).} Both terms are
properties of the condensate (vacuum) itself, not of any specific
soliton state within it. A specific hypothesis for what they represent:
the "bare" ($\beta=0$, $V=15/8$) and "self-consistent" ($\beta=\bst$,
$V=\vp$) states are the \emph{unbound} and \emph{bound} configurations
of the \emph{same} $Q_H{=}1\to Q_H{=}2$ fusion already responsible for
the electron mass (Theorem 6.11 of Paper~IV~\cite{Paper4}%\ref{P4:thm:second_spoke}
,
Theorem 6.3 of Paper~IV~\cite{Paper4}%\ref{P4:thm:spoke_ground}
: two $Q_H{=}1$ quanta fusing to the
$Q_H{=}2$ singlet). Under this reading, $4\pi$ is not a second,
independent topological transition requiring its own justification ---
it is the \emph{same} fusion event observed through a different
quantity (the vacuum's virial ratio, rather than the electron's mass):
one microphysical event with two observable footprints. This also gives
a structural reason for the terms to add rather than combine some other
way: the standard $\mathrm{SU}(2)_k$ WZW action itself splits as
$S_{WZW}=S_{\mathrm{kinetic}}+\frac{k}{12\pi}\int_B\mathrm{Tr}(g^{-1}dg)^3$
(smooth sigma-model term plus topological Wess--Zumino term, added, not
multiplied), and this framework already identifies the density-feedback
model exactly with this WZW theory (Theorem~\ref{P3:thm:bosonization}).
If the $\beta:0\to\bst$ relaxation is a path in this WZW configuration
space, its action would naturally split the same way: a kinetic
contribution ($15/8-\vp$, the virial ratio's classical relaxation
distance) plus a topological one ($4\pi\cdot\Delta Q$, $\Delta Q=1$).
This is a coherent picture, not a derivation: it has not been shown that
the WZW action, evaluated on this specific trajectory, actually produces
this decomposition rather than merely resembling its shape, and it has
not been checked whether the papers' own treatment of the $\beta=0$
state supports identifying it with the unbound $Q_H=1$ configuration.

\textbf{Status: unverified, and provably not exact.} $\Delta_1=(2136\vp-3392)/5$
is algebraic of degree $2$ over $\mathbb{Q}$ (Result~\ref{P3:prop:cascade}).
$4\pi+(15/8-\vp)$ is transcendental: $15/8-\vp\in\mathbb{Q}(\vp)$ is
algebraic, $4\pi$ is transcendental ($\pi$ transcendental,
Lindemann), and algebraic numbers are closed under addition, so their
sum is transcendental. An algebraic and a transcendental number are
never equal. Equation~\eqref{P3:eq:delta1_candidate} is therefore
provably not an exact identity, at any precision --- it is, at best, a
close numerical approximation. This is a numerically confirmed sum of
two independently-established quantities, not a derivation. No
calculation has yet been shown to produce $4\pi+(15/8-\vp)$ as a
single, unified expression relevant to $\Delta_1$ (e.g.\ from a
combined vacuum free energy or effective action encompassing both the
topological Chern--Simons sector structure and the classical relaxation
dynamics); given the algebraic/transcendental mismatch, such a
derivation --- if one exists --- would need to relate $\Delta_1$ to
$4\pi+(15/8-\vp)$ approximately, or identify a different,
transcendental quantity adjacent to $\Delta_1$ as the true target. This
remains a candidate, not a result, and should not be conflated with the
proved parts of this paper.
\end{remark}
...
relationship between them. A candidate physical origin for $\Delta_1$
itself, tying it to the condensate's own vacuum topology and relaxation
dynamics, is given in Remark~\ref{P3:rem:delta1_candidate}: since that
candidate ($4\pi+(15/8-\vp)$) is transcendental while $\Delta_1$ is
algebraic, it can only ever be an approximate relation
($0.0061\%$), not an exact one, consistent with, but not proving,
a genuine physical connection.
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{[Open]}
    ...
    A candidate decomposition has been found (Remark~\ref{P3:rem:delta1_candidate}):
    $\Delta_1 \approx 4\pi + (15/8-\vp) = 12.823\,337$, a $0.0061\%$
    match, using only already-established quantities (the $\Delta Q=1$
    Chern--Simons unit and the thin-torus-to-self-consistent virial
    relaxation distance). This is a numerically verified sum, not a
    derivation; no unified calculation yet produces it as a single
    expression.
