...
%──────────────────────────────────────────────────────────────────────
% TWO-CHANNEL (POLARIZATION × VERTEX-SUPPRESSION) CANDIDATE FOR Delta_1
% Status: exploratory hypothesis; bracketing evidence only — not established.
% Supporting scripts: paper3_support/ (two_channel_bracket.py reproduces the
% table below; 2D pair solver and checkpoints under paper3_support/pair_cell/).
%
% Hypothesis: the geo->golden step is carried jointly by two actions of the
% same condensate tubes on the photon,
%     aInv_obs = aInv_geo · eps_eff · <1/(1+beta X)>^{-1},
% where eps_eff is the 2D Maxwell-Garnett response of the T(2,2) tube pairs
% (polarization channel) and <1/(1+beta X)> is the mean coupling suppression
% (vertex channel, S_eff ~ 1/(1+beta rho) as in Papers IV/VII). With hopfions
% at CONTACT packing (spacing 2(R0+1)=8 condensate units) there is no free
% density: both factors are fixed by the same geometry.
%   pair separation s = 2R0/(C2* C*) = 0.96231 solver units;
%   <alpha_pair> from the validated 2D anisotropic solver (h=0.1, eta=1e-2):
%     Model A (incoherent X1+X2):      -1.850
%     Model B (coherent gradient sum): -2.953
%   vertex deficit per tube D = 2·pi·b*·4·arctan(x*)/x* = 1.45640
%     (closed form; the same integral as J_fb).
% Result against the target golden/geo = 112.5/phi^10 = 0.914695:
%   square lattice:     A: 0.97178 (+6.2%)   B: 0.86571 (-5.4%)
%   triangular lattice: A: 0.96974 (+6.0%)   B: 0.84797 (-7.3%)
%**The two-channel picture.** The dielectric bridge as originally posed made the polarization channel carry the entire geo→golden shift; that's what forced the dilute n_req and the unexplained 1.5×-beyond-contact spacing. But the framework already contains a second action of the same tube on the coupling: the vertex suppression S_eff ∝ 1/(1+βρ) (the chameleon/Vainshtein structure of Papers IV/VII). The photon between two charges then sees *both*: the medium's polarization (ε_eff, overshooting at contact density) and the coupling suppression from the tubes' own density (pushing α⁻¹ the other way). Written as α⁻¹_obs = α⁻¹_geo · ε_eff · ⟨1/(1+βX)⟩⁻¹, **contact packing stops being excluded and the whole construction becomes parameter-free**: the same geometry fixes both factors, and the suppression average even has a closed form (it's Paper III's own J_fb integral — deficit 4·arctan(x*)/x* per tube).
%
%At contact packing of the T(2,2) pairs, with the phase-1 ⟨α_pair⟩ values:
%
%| lattice    | model | ε_eff  | vertex | product | vs 112.5/φ¹⁰ |
%| ---------- | ----- | ------ | ------ | ------- | ------------ |
%| square     | A     | 0.8258 | 1.1768 | 0.9718  | **+6.2%**    |
%| square     | B     | 0.7357 | 1.1768 | 0.8657  | **−5.4%**    |
%| triangular | A     | 0.8015 | 1.2099 | 0.9697  | +6.0%        |
%| triangular | B     | 0.7009 | 1.2099 | 0.8480  | −7.3%        |
%
%The two superposition models **bracket the golden-angle target from both sides at ±6%**, and the lattice choice is nearly irrelevant. That's not a match — flags: the vertex normalization (exponent 1 on the suppression average) is my assumption pending a check against Paper IV's actual vertex structure, and A/B are toy backgrounds. But it's the first construction in this program with *zero free densities* that lands within single-digit percent of the target, and the sign structure is exactly your "both actions on the tube."
%
%In Model B the outward gradients cancel exactly at the midpoint (X_B = 0, ε = 1 there — a positive-ε lens), with the negative-ε region a single band *surrounding* the pair; Model A instead has one merged negative core containing both centers and the midpoint. The real T(2,2) field will pick one behavior — which is now the decisive computation: build the **actual coherent two-tube background** (correct relative winding from the papers' construction, not the ± toy superpositions), get the single ⟨α_pair⟩ and the same-field ⟨1/(1+βX)⟩, and the two-channel product at contact is then fully determined. If it lands on 112.5/φ¹⁰ at sub-percent, OP4's Δ1 has a mechanism.
% The two background models bracket the target from both sides; the lattice
% choice is subdominant. Known gaps: (i) the vertex normalization (exponent
% of the suppression average) is assumed, not derived from the Paper IV
% vertex; (ii) A/B are toy superpositions — the physical coherent T(2,2)
% background selects a single value between them; (iii) Re(alpha) is grid-
% converged at the ~1% level only, Im(alpha) not converged (resonant annulus
% under-resolved; see paper3_support/pair_cell/results.md).
% Decisive computation: <alpha_pair> and <1/(1+beta X)> on the true coherent
% T(2,2) background at contact packing — fully parameter-free; a sub-percent
% landing on 112.5/phi^10 would identify the Delta_1 mechanism of Open
% Problem 4.
%──────────────────────────────────────────────────────────────────────
