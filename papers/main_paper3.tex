...
two-channel route. A first coarse-grid evaluation on the exact coherent
background lands between the toy brackets, at $+3$ to $+4\%$ of the
target, with a genuinely reduced suppression deficit ($2.39$ per pair
against $2.91$ for independent tubes), but is not grid-converged: the
coherent background destabilizes under-resolved grids well before the
toy backgrounds do, and the matched grid/absorption ladder it requires
has not yet been run. The two-channel route therefore remains open ---
neither confirmed at the sub-percent level nor refuted.
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{[Open]}
    ...
    action change; it remains a numerical observation. What is missing
    is a derivation of why $\aInv_{\mathrm{geo}}$ should be displaced
    from the golden-angle attractor by a vacuum action at all, and
