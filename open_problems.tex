...
The WZW action along the vacuum relaxation path is exactly computable
(Remark~P3:rem:delta1\_candidate): per unit topological charge,
$4\pi+\mu^*(15/8-\vp)=4\pi+(53-31\vp)/8=12.921\,489$, differing from
$\Delta_1$ by $0.76\%$; the numerically closer bare-travel variant
$4\pi+(15/8-\vp)=12.823\,337$ ($0.0061\%$) omits the coupling $\mu^*$ and is
not the model's action change. Both are transcendental, $\Delta_1$ algebraic,
so no exact identity exists. Still open.
...
\item \textbf{[P11:OP2] Higher ionisation energies on the spiral --- \emph{resolved (Paper~XI)}.}
Paper~XI originally used only the first ionisation energy (IE) of each
element. This is closed within Paper~XI itself by the master formula
$\nlvl{\IE_k}=\nlvl{\ERy}-\log_\vp Z_{\rm eff}(k)+\log_\vp n_{\rm qn}(k)$
(Proposition~\ref{P11:prop:master_formula}), which places every
successive IE on the $\vp$-spiral with no new parameters. Shell-completion
IEs land within $|\Delta n|\le0.017$ of integer tower levels
...
(Proposition~\ref{P11:prop:inner_degen}). Whether the shell-boundary
jump sizes have exact $\vp$-ratio structure remains open.
\source{P11:prop:master_formula, P11:prop:shell_completion, P11:prop:inner_degen}
