...
indicating the four-term formula is complete within the condensate framework. A second-order measurement-backreaction account of the remaining residual (rectification of probe driving by the feedback vacuum; exploratory) is recorded in Paper~III~\cite{Paper3}. Corollary~\ref{P4:cor:four_term_unconditional}, makes this four-term formula unconditional.
\qed
\end{proof}
...
\noindent
The first row's shift $\Delta_1 = 12.824$ is the exact algebraic
difference of two independently exact endpoints; it is not a running
effect and no carrier is derived in this paper. The vacuum-response
carrier search of Paper~III~\cite{Paper3} (Open Problem~4 there) is
complete and negative, and the golden-angle attractor is identified
there as the anyonic observable
$360/\vp^2 = k\,|2I|\operatorname{Tr}[\rho\,U_{\mathrm{mono}}]$; the
remaining open step is definitional (why the coupling counts the
condensate's angular quanta), not dynamical.
