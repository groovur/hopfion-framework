...
Two structural results follow: shell-completion IEs land within $0.017$
of integer tower levels (Proposition~\ref{P11:prop:shell_completion}),
with xenon marking consecutive integers ($-12$, $-13$) at successive
shell strippings; and the inner-shell pair O~IE$_6$--Na~IE$_5$
is near-degenerate at $|\Delta n|=0.0021$
(Proposition~\ref{P11:prop:inner_degen}),
the exact inner-shell analogue of the H--O outer-shell result.
...
Section~\ref{P11:sec:successive_IE} extends the spiral to successive
ionisation energies and proves the inner-shell O--Na near-degeneracy.
...
\item The denominator $4\cdot2\pi$ is the Chern--Simons phase
  $4\pi$ (Paper~IV~\cite{Paper4}, Theorem~6.11) %\ref{P4:thm:second_spoke}
  divided by $2\pi$ (the single-winding normalisation).
\item The numerator $k=3$ is the WZW level of the $\mathrm{SU}(2)_3$
  condensate (Paper~I).
\end{itemize}
This connects the $0.003\%$ residual of the H--O near-degeneracy
to the same WZW Chern--Simons structure that governs the electron
mass formula of Paper~IV~\cite{Paper4}.
...
The O IE residual ($0.003\%$) is smaller than $\aem/\pi$ ($0.23\%$)
by the factor $3/(8\pi)\approx 0.12$ relating the
atomic orbital shift coefficient $k/(8\pi)$ to the
full QED one-loop correction.
\end{remark}
...
\begin{proof}
Apply equation~\eqref{P11:eq:master_formula} with
$Z_{\rm eff}(k)=n_{\rm qn}(k)\sqrt{\IE_k/\ERy}$
(inverted from the hydrogenic formula) and principal quantum numbers
determined by the sub-shell being stripped.
...
Propositions~\ref{P11:prop:R_equals_1}--\ref{P11:prop:Zeff_closed})
arises because oxygen's outermost electron sees an effective charge
that forces $\IE(\mathrm{O})\approx\ERy$.  The multi-layer spiral
contains an exact inner-shell analogue at the $2s$-stripping level.
...
($Z_{\rm O}=8$ vs $Z_{\rm Na}=11$) is almost exactly compensated
by the additional screening from the extra $2p^5$ electrons that
sodium has contributed to its $1s^2$ core relative to oxygen's
$1s^2$ core: sodium has three more nuclear protons but its ion
Na$^{4+}$ exposes a $2s$ electron from a $1s^2\,2s^2\,2p^1$
configuration while O$^{5+}$ exposes a $2s$ electron from
$1s^2\,2s^1$, and the combined effect places the binding energies
within $0.28\,\mathrm{eV}$ of each other.
\qed
\end{proof}
...
\item \textbf{[Closed] Higher ionisation energies on the spiral.}
Resolved in Section~\ref{P11:sec:successive_IE}.
...
(Proposition~\ref{P11:prop:inner_degen}).
Whether the shell-boundary jump sizes have exact $\vp$-ratio structure
remains open.
...
Shell completions land near integer spiral levels: Pb~IE$_3$ at $\Delta n=0.011$
from $-13$, Xe~IE$_3$ at $\Delta n=0.005$ from $-13$, Xe~IE$_1$ at
$\Delta n=0.017$ from $-12$ (Proposition~\ref{P11:prop:shell_completion}).
Xenon thus marks integer positions on successive layers of the multi-layer
spiral, extending its role as a noble-gas boundary marker
(Section~\ref{P11:sec:elements}).
The inner-shell pair O~IE$_6$--Na~IE$_5$ is near-degenerate at
$|\Delta n|=0.0021$ (Proposition~\ref{P11:prop:inner_degen}), comparable
to the H--O first-IE result and arising from the same mechanism: two
elements exposing the same sub-shell electron at the same effective
charge.  The spiral crossing predicted by this near-degeneracy
