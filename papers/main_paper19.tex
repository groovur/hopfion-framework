...
of the Jones-polynomial computation.  The shared modulus is the
reciprocal virial $1/V^*$, with $V^* = S_{0,1/2}/S_{0,0} = \vp$ proved
by Witten bosonization (Paper~III~\cite{Paper3}); the same $V^*$ carries
the fine-structure constant ($\aem^{-1}=k|2I|/V^{*2}$) and the
cosmological-constant normalisation ($c=\vp^3+V^{*-2Q_H}$,
Paper~VII~\cite{Paper7}), so the coincidence of moduli is one instance
of a single proved modular constant recurring across sectors rather than
an isolated accident --- though, as stated, no dynamical identity between
the mixing angle and the knot invariant is implied.
...
the bare ``permitted/forbidden'' classification. Their moduli are
reciprocals, $|u_1|=1/\vp$ and $|u_2|=\vp$,
so
\begin{equation}
