...
\residual{exact}
\begin{proof}
From Lemma~\ref{P4:lem:virial_factor}: $V(0;\,R_0) = \tfrac{15}{8}(1+\tfrac{1}{2R_0^2})
= \tfrac{15}{8}\cdot\tfrac{2R_0^2+1}{2R_0^2}$.
From Theorem~\ref{P4:thm:mass_renorm}: $V^*/V(0;\,R_0) = \tfrac{8\vp}{15}\cdot\tfrac{2R_0^2}{2R_0^2+1}$.
Therefore
\begin{equation}
  V^*(R_0)
...
\end{equation}
The factors $(2R_0^2+1)/2R_0^2$ and $2R_0^2/(2R_0^2+1)$ are reciprocals
and cancel exactly, for all $R_0>0$, not only in the thin-torus limit.
\end{proof}
\status{published}
\source{P4:thm:verlinde_cancel}
\residual{exact}
...
The $0.26\%$ Verlinde piece in the residual decomposition is not a
physical correction to the spoke formula (Corollary~\ref{P4:cor:verlinde_exact}).
...
\end{equation}
so the corrected formula $m_e/\Lcond = \vp^{20}e^{4\pi-3/400}$ is exact
in the continuum thin-torus limit, with the $0.22\%$ residual at $R_0=3$
entirely attributable to the finite grid $h=0.12$.
\status{published}
\source{P4:cor:verlinde_exact}
