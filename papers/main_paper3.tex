...
is a leading-order prediction ($0.004\%$ from measured), with a
...
\begin{enumerate}[noitemsep]
  \item \textup{(Golden-angle cascade)}
    ...
    \begin{equation*}
      ...
      \;\xrightarrow{\text{exact difference, Sec.~\ref{P3:sec:alpha}}}\;
      \underbrace{\dfrac{360}{\vp^2}}_{\displaystyle 137.508}
      \;\xrightarrow{\text{WZW anti-screening (one-loop)}}\;
      \underbrace{\aInv}_{\displaystyle 137.036}
    \end{equation*}
    where $360/\vp^2$ is the golden angle in degrees
    and both endpoints of the first step are independently exact
    (Result~\ref{P3:prop:cascade}); only the second step is a
    WZW running effect.
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{The fine structure constant.}  The same WZW geometry produces a
    two-step cascade: the value $\vp^6/\sin^4(\pi/5) = 150.3$ and the
    golden-angle attractor $360/\vp^2 = 137.508$ are both independently
    exact, and their (exact, computed) difference is $\Delta_1$
    (Result~\ref{P3:prop:cascade}) --- not a WZW running effect.
    A further, one-loop WZW correction then gives the exact formula
    \begin{equation*}
  ...
  \item \textbf{The numerical residuals and gap.}
    ...
    closed by the $T$-matrix and Chern--Simons corrections of
    Paper~IV~\cite{Paper4}, not by a UV/IR scale ratio.  The value
    $\Delta_1 = 12.824$ appearing in the two-step cascade above is a
    separate, independent quantity: both cascade endpoints are exact, their
    exact difference is $\Delta_1$, and its own physical origin remains
    open (Open Problem~4, Remark~\ref{P3:rem:delta1_candidate}).
\end{enumerate}
...
\begin{enumerate}[noitemsep]
  ...
  \item The second step ($\Delta_2 = k/(2\pi) = 0.477$, $3.5\%$) is the
    one-loop WZW correction at the icosahedral scale, and remains an
    exact one-loop running effect (Knizhnik--Zamolodchikov
    exactness).
\end{enumerate}
...
same icosahedral origin, is Open Problem~4 of Section~\ref{P3:sec:open}.
\end{result}
...
fusion that generates the electron mass (Theorem 6.11 of
Paper~IV~\cite{Paper4}%\ref{P4:thm:second_spoke}
) --- the Abelian Chern--Simons unit adds $4\pi$, giving the total
...
\paragraph{The dielectric route to $\Delta_1$.}
A structural alternative to an action-valued $\Delta_1$ is a medium
interpretation: the two ends of the first cascade step are coupling
values of the same charge pair measured through different states of the
feedback vacuum,
\[
  \aInv(\bst) \;=\; \aInv(0)\cdot\epsilon_{\mathrm{eff}},
  \qquad
  \epsilon_{\mathrm{eff}}
  \;=\; \frac{360/\vp^2}{\vp^6/\sin^4(\pi/5)}
  \;=\; \frac{112.5}{\vp^{10}}
  \;=\; 0.914\,695 ,
\]
an exact algebraic target --- so, unlike any $4\pi$-containing
expression, an exact closure is not excluded by transcendence. The
fluctuation medium of the feedback Lagrangian $L(X) = X/(1+bX)$ (the
integrand of $\Jfb$, with $b = \beta/C^2$ as in
Proposition~\ref{P3:prop:virial_exact}) around a tube background of
kinetic density $X$ is the anisotropic dielectric
\[
  \epsilon_{\tan} = \frac{1}{(1+bX)^2},
  \qquad
  \epsilon_{\mathrm{rad}} = \frac{1-3bX}{(1+bX)^3},
\]
with the anisotropy axis along the background gradient. At $b=0$ the
medium is exactly uniform ($\epsilon_{ij}=\delta_{ij}/\vp^6$), so the
ultraviolet endpoint of the cascade needs no medium correction; the
entire step must be carried by the relaxed vacuum at $b=b^*$
(Corollary~\ref{P3:cor:fixedpoint_exists}).

\paragraph{Zero-frequency vacuum resonance for $b>1/24$.}
$\epsilon_{\mathrm{rad}}$ changes sign where $3bX=1$. For the unit tube
($X = 8/(1+\rho^2)^2$, so $X(0)=8$) a negative-$\epsilon_{\mathrm{rad}}$
core $\rho<\rho_c$ with $(1+\rho_c^2)^2 = 24b$ exists precisely for
$b > 1/24$; at the fixed point $\rho_c(b^*) = 0.519$. A validated
shooting computation of the static cell polarizability under limit
absorption ($\epsilon\to\epsilon+i\eta$, $\eta\to0^+$) gives
\[
  \alpha_{\mathrm{pol}}(b^*) \;=\; -0.971\,07 \,+\, 0.013\,53\, i .
\]
The negative real part is anti-screening --- the correct direction for
$\aInv_{\mathrm{geo}} \to 360/\vp^2$. The nonzero imaginary part at
zero frequency appears only above the threshold (the same computation
at $b^*/2$ gives $\mathrm{Im}\,\alpha_{\mathrm{pol}} \to 0$ exactly as
$\eta\to0$): the relaxed vacuum is anomalously dissipative to static
perturbations, a zero-frequency resonance of its
negative-$\epsilon_{\mathrm{rad}}$ core. This is a parameter-free
qualitative consequence of the feedback model; the vacuum sits above
the threshold but not far above it ($b^* = 0.0672$ against
$1/24 = 0.0417$).

\paragraph{The tube-density requirement and its status.}
In a dilute (Maxwell--Garnett) medium of such cells,
$\epsilon_{\mathrm{eff}} =
1 + 2n\alpha_{\mathrm{pol}}/(1-n\alpha_{\mathrm{pol}})$, the target
value is met at areal cell density $n = 0.0459$ per $C^{*2}$
($C^{*2} = \tfrac{3}{4}\vp^5/2^{4/3}$), equivalently $0.0139$ tubes per
condensate unit area, i.e.\ a mean spacing of about $12$ condensate
units between $Q=2$ hopfions. The documented $T(2,2)$ two-tube geometry
(cross-section radius $1/C_2^*$, separation $d = 2R_0/C_2^* = 1.748$,
ratio $2R_0$ exactly) with hopfions at contact (spacing $2(R_0+1)=8$)
gives a density $2.2$--$3.9\times$ larger, and no inter-hopfion spacing
is specified anywhere in this series: within the single-channel
dielectric route the density is an underived parameter, not a
prediction. A two-dimensional anisotropic solve of the two-tube cell at
the documented separation shows the pair does not merge into a weaker
inclusion --- the orientation-averaged pair polarizability is
$-1.815$ for the incoherent superposition ($0.935$ of twice the single
tube) and $-2.991$ for the coherent gradient superposition ($1.540$ of
twice the single tube) --- so contact packing cannot be reconciled
within the polarization channel alone.

\paragraph{Two-channel candidate (exploratory).}
The framework contains a second coupling response to the same tubes:
the vertex suppression $S_{\mathrm{eff}} \propto 1/(1+\beta\rho)$
(chameleon/Vainshtein structure; Paper~IV~\cite{Paper4} and
Paper~VII~\cite{Paper7}). If both channels act,
\[
  \aInv_{\mathrm{obs}} \;=\; \aInv_{\mathrm{geo}}\cdot
  \epsilon_{\mathrm{eff}}\cdot
  \bigl\langle 1/(1+bX)\bigr\rangle^{-1},
\]
then at contact packing there is no free density: both factors are
fixed by the same geometry, and the suppression average is given in
closed form by the $\Jfb$ integral (deficit
$2\pi b^*\cdot 4\arctan(x^*)/x^* = 1.456$ per tube, $x^*=\sqrt{8b^*}$).
With the computed pair polarizabilities the product brackets the target
from both sides: $0.975$ ($+6.6\%$) for the incoherent and $0.862$
($-5.7\%$) for the coherent superposition on a square lattice, with
triangular-lattice values $+6.5\%$ and $-7.7\%$. Epistemic status:
exploratory. The vertex normalization (first power of the suppression
average) is assumed rather than derived; the two superpositions are toy
backgrounds bracketing the physical coherent $T(2,2)$ field; and the
computed imaginary parts are not grid-converged (the
$\epsilon_{\mathrm{rad}}=0$ annulus must be resolved on a scale that
shrinks with $\eta$). The deciding computation --- both factors
evaluated on the exact coherent two-tube background, the degree-two
rational map at the documented separation --- is parameter-free: a
sub-percent landing on $112.5/\vp^{10}$ would identify the $\Delta_1$
mechanism sought by Open Problem~4, and a clear miss would refute the
two-channel route. The converged evaluation on the exact coherent
background (quadrant ladder to $h=0.025$; anisotropy-prescription
sensitivity about $3\%$) gives a product $+3.3$ to $+5.1\%$ above the
target, with the suppression deficit genuinely reduced ($2.39$ per
pair against $2.91$ for independent tubes): not a sub-percent landing.

\paragraph{Resolution status: the null hypothesis.}
Every carrier mechanism computed for $\Delta_1$ fails its
pre-registered criterion: the relaxation action ($0.76\%$,
Remark~\ref{P3:rem:delta1_candidate}); every natural closed-form
medium average at the fixed point (each provably transcendental at
$b^*$ except the defining identity $V(b^*)=\vp$ itself); the
two-channel product at contact packing ($+3$ to $+7\%$ across
backgrounds); and a mutually linked vacuum network pinned at its
topological dilation bound (two linked $R_0$-circles must sit closer
than $2R_0$, so a repulsive linked network equilibrates at spacing
$2R_0$ exactly) --- computed by three-dimensional periodic
homogenization of a linked Hopf-chain cell, grid-converged across
four resolutions: $P = 0.273$--$0.281$ against $0.9147$, a factor
$3.3$ miss; the threshold-density network (volume-mean suppression
$U \approx 0.6$) over-responds by far. The effective response is
monotone in the network density and no framework-native density lands
on the target: the dilute density that would fit ($12$-unit hopfion
spacing) is underived, while both derivable densities (contact,
$2(R_0+1)$, and the linking threshold, $2R_0$) miss by far more than
their modeling systematics. The surviving reading is the null
hypothesis: the two ends of the first cascade step are independently
exact --- $\vp^6/\sin^4(\pi/5)$ from the single-soliton geometry,
and $360/\vp^2$ as the anyonic observable
$k\,|2I|\operatorname{Tr}[\rho\,U_{\mathrm{mono}}]$ of the
$2I$-ordered condensate (following paragraph) --- and $\Delta_1$ is
their bookkeeping difference, not a response of the vacuum: the
physical coupling never sits at the geometric value; the condensate
ground state realizes the statistical value directly. Under this
reading, Open Problem~4 reduces to the definitional step of the
anyonic identification --- why the coupling counts units of the
condensate's angular quantum, the quantum itself being fixed at
$1/(k|2I|)$ of the cycle by the orbifold twist spectrum --- which
subsumes the origin of $\Delta_1$ without requiring a carrier.
...
\paragraph{The attractor as an anyonic observable.}
The golden-angle attractor is a closed-form quantity of the
$\mathrm{SU}(2)_3$ modular data. The fusion $\tfrac12\otimes\tfrac12
= 0 \oplus 1$ has monodromy angles $2\pi(h_c - 2h_{1/2})$ equal to
$-108^\circ$ and $+36^\circ$ in the two channels, and Born weights
$p_c = d_c/d_{1/2}^2$: $p_0 = 1/\vp^2$, $p_1 = 1/\vp$, with
$p_0 + p_1 = 1$ exactly. The condensate charge
$Q = 10 = \mathrm{ord}(q_{2I})$ closes both monodromies
simultaneously ($10 \times 36^\circ$ with winding $+1$;
$10 \times (-108^\circ)$ with winding $-3$), so the fundamental
winding-one closure is one full turn. The monodromy expectation in
the exchange state is exactly real and equals the vacuum-channel
weight:
\[
  \operatorname{Tr}\bigl[\rho\, U_{\mathrm{mono}}\bigr]
  = \frac{1}{\vp^2}\,e^{-i\,108^\circ}
  + \frac{1}{\vp}\,e^{+i\,36^\circ}
  = \frac{3-\sqrt5}{2}
  = \frac{1}{\vp^2}
  = \frac{S_{\frac12\frac12}\,S_{00}}{S_{0\frac12}^{2}},
\]
the monodromy scalar of anyonic interferometry (the fringe-visibility
factor of interfering anyonic charges): the imaginary parts of the
two channels cancel identically, so the vacuum-channel weight emerges
from a single operator expectation, not from a channel projection
imposed by hand. With the pure-number identity $360 = k\,|2I| = 3
\times 120$ (the same $k \times (\text{group invariant})$ vocabulary
as the $Q/(k\,h(E_8))$ structure of the loop term in
Paper~IV~\cite{Paper4}), the attractor takes a form free of any
angular convention:
\[
  \frac{360}{\vp^2}
  \;=\; k\,|2I| \cdot
  \operatorname{Tr}\bigl[\rho\, U_{\mathrm{mono}}\bigr].
\]
All three factors are derived objects: the forced level $k=3$, the
ground-state group order $|2I| = 120$, and the exact interferometric
scalar. By contrast, the geometric endpoint $16\vp^8/5$ admits no
expression in this modular vocabulary (an exhaustive scan of the
natural $S$-matrix, quantum-dimension, twist, and fusion-probability
forms yields no exact match): the two ends of $\Delta_1$ are
quantities of different algebras --- soliton geometry and anyon
statistics --- consistent with $\Delta_1$ having no dynamical
carrier. The normalization of this identity is fixed by the
paragraph that follows.

\paragraph{The angular quantum from the orbifold spectrum.}
The unit in $360 = k\,|2I|$ is itself spectral. The physical modular
phase of a primary of weight $h$ is $\exp(2\pi i(h - c/24))$, with
$c = 3k/(k+2) = 9/5$, so $c/24 = 27/360$. The four untwisted
$\mathrm{SU}(2)_3$ primaries ($h \in \{0, 3/20, 2/5, 3/4\}$)
generate the phase group $\mathbb{Z}_{40}$. Orbifolding by the
ground-state symmetry $2I$ adds the twisted sector of the McKay--WZW
theory (the Coxeter twisted-sector theorem of
Paper~V~\cite{Paper5}%\ref{P5:thm:h2I_coxeter}
), with primaries labeled by the Coxeter exponents
$m \in \{1,7,11,13,17,19,23,29\}$ of $E_8$ and weights
$h_m = m/(k\,h(E_8)) = m/90$; their physical phase exponents are
$(4m-27)/360$. The $m=7$ primary has phase exponent exactly $1/360$,
and the full phase group is exactly $\mathbb{Z}_{360} =
\mathbb{Z}_{k|2I|}$: orbifolding by $2I$ refines the phase lattice
from $\mathbb{Z}_{40}$ to $\mathbb{Z}_{k|2I|}$, with the one-degree
quantum generated by an actual primary field. The refinement requires
the framing shift $-c/24$ (without it the twisted exponents $m/90$
generate only $\mathbb{Z}_{90}$). The angular quantum of the
$2I$-ordered condensate --- the minimal phase its own modular data
distinguishes --- is therefore $1/(k|2I|)$ of the cycle, and
$\aInv = k\,|2I| \cdot \operatorname{Tr}[\rho\,U_{\mathrm{mono}}]$
reads: the coupling is the number of condensate phase quanta spanned
by the vacuum-weighted monodromy cycle. The remaining definitional
step is why the coupling counts these units --- the identification of
$\aInv$ with a phase-count of the orbifold spectrum.

\paragraph{Measurement backreaction on the coupling (exploratory).}
In the anyonic reading, a measurement of the coupling is a monodromy
interferometry event. The iterated measurement map is an exact
martingale in the fusion weights: repeated measurement cannot shift
the mean of $\operatorname{Tr}[\rho\,U_{\mathrm{mono}}]$ at any order
in the fringe strength, with or without linear relaxation toward the
Born state; the measured coupling acquires fluctuations (standard
deviation $\propto t/\sqrt{g}$ for fringe strength $t$ and
re-preparation rate $g$) but no bias. A bias does arise at second
order, through the medium rather than the state: the fluctuation
response of the feedback vacuum rectifies periodic probe driving,
\[
  \langle \epsilon(X_0 + \delta X) \rangle
  = \epsilon(X_0) + \tfrac{1}{4}A^2 f^2\,\epsilon''(X_0) + O(A^4),
  \qquad
  \epsilon_{\tan}'' = \frac{6b^2}{(1+bX)^4} > 0,
\]
shifting the cell polarizability by $r = C_{\mathrm{rect}}\,A^2$ with
$C_{\mathrm{rect}} = -0.00218 + 0.00063\,i$ at $b^*$. The sign is the
one required to move the four-term formula of
Paper~IV~\cite{Paper4} toward the measured value; accounting for the
full residual $\aInv^{\mathrm{meas}} - \aInv^{\mathrm{formula}} =
137.035\,999\,177 - 137.035\,998\,486 = +6.9\times10^{-7}$ (Paper~IV's
four-term value against the measured $\aInv$) would require drive
amplitude $A \approx 0.005$
--- perturbative, and an inference rather than a fit. The resonant
shell of the relaxed vacuum enhances the rectification by only a
factor $1.6$ ($\epsilon_{\mathrm{rad}}''$ is smooth at the
$\epsilon_{\mathrm{rad}}=0$ shell), so the effect is a second-order
drift, not an instability. The falsifiable signature is a
probe-strength dependence of the measured coupling at the
$10^{-6}$ level.
...
as we run to lower energies, meaning the coupling strengthens).  This
equation is used once in this paper: with
$\log(\mu_{\mathrm{UV}}/\mu_{\mathrm{IR}})=1$ it gives the one-loop
step $\Delta_2=k/(2\pi)$ from the golden angle to the physical value,
Eq.~\eqref{P3:eq:alpha_formula}.

It is \emph{not} used to derive $\Delta_1$. No $\mu_{\mathrm{UV}}$ or
$\mu_{\mathrm{IR}}$ is independently defined or computed here as a
physical energy scale, and the WZW beta function is not applied to
derive $\Delta_1$'s value. As established in Result~\ref{P3:prop:cascade}
above, both $\vp^6/\sin^4(\pi/5)$ and $360/\vp^2$ are independently
exact, so there is no scale ratio to derive: $\Delta_1$ is their exact
difference, not a running effect.
...
prediction; gap~(B) reflects the fact that $\Delta_1=12.824$ is the exact
difference between two independently-exact quantities
($\vp^6/\sin^4(\pi/5)$ and $360/\vp^2$, Result~\ref{P3:prop:cascade}),
not an RG-running effect awaiting derivation. Whether this numerical
proximity has a deeper explanation remains Open Problem~4.
\end{remark}
...
(Theorem 5.9 of Paper~I~\cite{Paper1})%\ref{P1:thm:charge_q10}
, and $36^\circ=\pi/5$ is the same
...
(Section~\ref{P3:sec:lsc_alpha_link}), closed by the $T$-matrix
correction (Theorem 5.1 of Paper~IV~\cite{Paper4})%\ref{P4:thm:three_term_alpha}
and the Chern--Simons correction (Theorem 5.3 of Paper~IV~\cite{Paper4})%\ref{P4:thm:four_term_alpha}
, not by a UV/IR scale ratio.
...
two-term $\alpha$ prediction: the $T$-matrix and Chern--Simons
corrections of Paper~IV~\cite{Paper4} are required for the full result.
...
$T$-matrix and Chern--Simons corrections of Paper~IV~\cite{Paper4} (not
by a UV/IR scale ratio).  They are not the same gap in different notation.
\end{remark}
...
is a genuine formula residual, closed by the $T$-matrix and
Chern--Simons corrections of Paper~IV~\cite{Paper4}.
...
of~\cite{Paper1}), and Theorem 6.1 of Paper~I~\cite{Paper1}%\ref{P1:thm:vacuum}
\ proves the $Q=2$
...
resonance in packing problems. This is the same mathematical fact responsible
...
physical connection.  Independently, $\vp = V^{*} = S_{0,1/2}/S_{0,0}$ is
the WZW virial value proved by bosonization
(Section~\ref{P3:sec:bosonization}), so the denominator is $\vp^2=V^{*2}$
and $1/\vp^2 = \operatorname{Tr}[\rho\,U_{\mathrm{mono}}]$ is the vacuum
Born weight: $360/\vp^2 = k|2I|/V^{*2}$ is the anyonic observable of
Remark~\ref{P3:rem:packing_layers}, a modular grounding of the denominator
complementing the packing one.
...
The one-loop WZW correction has the \emph{opposite} sign to ordinary QED
screening. This is because the condensate is an $\mathrm{SU}(2)_k$ WZW
model, which is non-abelian. Non-abelian gauge theories are
asymptotically free: the coupling weakens at high energies and
strengthens at low energies. The WZW one-loop term anti-screens:
virtual flux-knot pairs enhance the coupling at lower energies, reducing
$\aInv$ by $k/(2\pi)$ from the golden angle ($137.508$) to the physical
value ($137.036$).

This is the same mechanism as asymptotic freedom in QCD, but for the
electromagnetic coupling in the icosahedral condensate background. It
governs the one-loop step ($360/\vp^2\to\aInv$) only. The earlier,
larger step ($150.3\to137.508$) is not a running effect at all --- both
endpoints are independently exact (Result~\ref{P3:prop:cascade}).
...
$\Delta_1$ has the exact closed form $\Delta_1 = (2136\vp-3392)/5$
(derived from $\vp^6=8\vp+5$ and $(3-\vp)^2=5(2-\vp)=5/\vp^2$), confirming
it is algebraic of degree $2$ over $\mathbb{Q}$, i.e.\ an element of
$\mathbb{Q}(\sqrt5)$. Since $\Delta_2=k/(2\pi)$ is transcendental ($\pi$
is transcendental), the ratio $\Delta_1/\Delta_2 = 2\pi(2136\vp-3392)/(5k)$
is transcendental and has no expression as a pure power of $\vp$ (the
closest integer power is $\vp^7\approx29.0$, versus the actual
$\Delta_1/\Delta_2=27.18$); this is expected given the different
algebraic character of $\Delta_1$ and $\Delta_2$, not evidence against a
relationship between them. The WZW action along the
condensate's relaxation path is computed exactly in
Remark~\ref{P3:rem:delta1_candidate}: $4\pi + \mu^*(15/8-\vp) =
12.921$, which differs from $\Delta_1$ by $0.76\%$; the numerically
closer bare-travel variant $4\pi+(15/8-\vp)$ ($0.0061\%$) omits the
coupling $\mu^*$ and is not the model's action change. Both are
transcendental while $\Delta_1$ is algebraic, so any such relation can
only be approximate, never exact.

\subsection{The residual, and its resolution in Paper~IV}
...
This residual is closed in Paper~IV by the $T$-matrix correction
$T_{1/2}/Q=3/400$ (three-term formula, $0.0004\%$) and the
Chern--Simons correction to the QED loop (four-term formula,
$5\times10^{-7}\%$) --- not by a UV/IR scale ratio, which plays no role
in this particular residual. Open Problem~4 concerns the separate and
much larger quantity $\Delta_1=12.824$ (Result~\ref{P3:prop:cascade}),
not this $0.006$ residual.
...
density-feedback self-consistency condition (Lemma~\ref{P3:lem:bridge}).
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{[Resolved in Paper~IV~\cite{Paper4}]}
    Close the $0.004\%$ residual in $\aInv = 360/\vp^2 - k/(2\pi) = 137.030$
    versus the measured $137.036$.
  ...
  \item \textbf{[Open]}
    $\Delta_1=12.824$ is \emph{not} an RG-running effect: both
    $\vp^6/\sin^4(\pi/5)=16\vp^6/(3-\vp)^2$ and $360/\vp^2$ are
    independently exact (the former using the exact identity
    $\sin^2(\pi/5)=(3-\vp)/4$ and the exact $\mu^*=3-\vp=\sqrt{k+2}/\vp$,
    Theorem~\ref{P3:thm:V_exact_all_R0}). $\Delta_1$ is simply their
    computable difference; no UV/IR scale ratio is involved, since both
    endpoints are independently exact. What remains open is narrower:
    is there a deeper algebraic reason
    $16\vp^6/(3-\vp)^2$ and $360/\vp^2$ should be close (both being
    built from $\vp$ and $k=3$), or is the $9.7\%$ proximity a numerical
    coincidence between two unrelated exact results?
    The WZW action along the vacuum relaxation path is exactly
    computable (Remark~\ref{P3:rem:delta1_candidate}): per unit
    topological charge, $4\pi + \mu^*(15/8-\vp) = 4\pi + (53-31\vp)/8
    = 12.921\,489$, which differs from $\Delta_1$ by $0.76\%$. The
    numerically closer variant $4\pi+(15/8-\vp) = 12.823\,337$
    ($0.0061\%$) omits the coupling $\mu^*$ and is not the model's
    action change; it remains a numerical observation. The vacuum-response carrier search for $\Delta_1$ is complete
    and negative (relaxation action; exact medium averages; two-channel
    polarization--vertex product; linked vacuum network at the $2R_0$
    dilation bound, by 3D periodic homogenization), and the attractor
    is identified exactly as the anyonic observable $360/\vp^2 =
    k\,|2I|\operatorname{Tr}[\rho\,U_{\mathrm{mono}}]$. The angular
    quantum is fixed by the orbifold twist spectrum (phase group
    $\mathbb{Z}_{360}$, generated by the $m=7$ Coxeter primary); the
    problem reduces to the definitional step that the coupling counts
    these units.
...
The cascade $\vp^6/\sin^4(\pi/5) \to 360/\vp^2 \to \aInv$ has two steps
of different character. The first, $\vp^6/\sin^4(\pi/5)\to360/\vp^2$, is
\emph{not} WZW anti-screening running --- both quantities are
independently exact (Result~\ref{P3:prop:cascade}), and their gap
$\Delta_1$ is a computed difference, not a derived running effect.
The second step, the one-loop correction
$k/(2\pi)=3/(2\pi)$, is a genuine WZW running effect.
The formula $\aInv = 360/\vp^2 - k/(2\pi) = 137.030$ agrees with the
measured $137.036$ to $0.004\%$ with no fitted parameters.
...
The $\alpha$ residual --- the formula $360/\vp^2 - k/(2\pi)
...
\href{https://doi.org/10.1017/CBO9780511666933}{doi:10.1017/CBO9780511666933}
