...
fluctuations confirming a gravitational sector at the condensate scale, with
the ultraviolet cutoff $\LUV=\MPl/\vp^6\approx0.0557\,\MPl$ set three
$\vp$-tower steps below the Planck mass;
the suppression $\Lambda_0 e^{-\vp^7/\beta}$, with a canonical leading normalisation $c=\vp^3$ refined by an anyonic $p_0^2$ correction, predicting $\approx10^{-122}\MPl^4$ in agreement with the observed $\Lobs$; CMB
...
and independently by chameleon screening at the framework's own
density-feedback coupling.
Dark matter is identified as the invisible Frank elastic strain of the
condensate director: a cold, pressureless orientation mode that clusters
on all scales, splay-unstable and self-organising into a disclination
that gives flat rotation curves with no fitted halo, tracking the baryons
and reproducing the radial-acceleration relation.
The Planck mass is fixed by the density-feedback flux; the condensate
ultraviolet cutoff $\LUV=\MPl/\vp^6\approx0.0557\,\MPl$ sits three steps below
it on the $\vp$-tower of Paper~VI, at the non-integer level $n_{\rm UV}\approx73.6$
...
Section~\ref{P7:sec:dark_matter} identifies dark matter as the invisible
Frank elastic strain of the condensate director, splay-unstable and
self-organising into disclinations that give flat rotation curves; the
strain tracks the baryons, reproducing the radial-acceleration
relation.
...
The ultraviolet cutoff sits three $\vp$-tower steps below the Planck mass,
$\LUV = \MPl/\vp^6\approx0.0557\,\MPl$ (Section~5,~\eqref{P7:eq:MPl}); the
one-loop Seeley--DeWitt term confirms that a gravitational sector is induced
at this scale.
\end{remark}
...
The specific golden-ratio value $m_g^2\propto\vp^{-8}$ is a Jordan-frame
quantity, tied to the non-canonical $S_{\mathrm{eff}}$ normalisation and
rescaled under the conformal map to the Einstein frame; no observable
depends on it, since only the smallness $m_g\ll\omega$ enters $v_{\mathrm{GW}}$
and holds for any such $m_g$.
...
\begin{equation}
  ...
  \label{P7:eq:1loop}
\end{equation}
The induced $R$-coefficient is of order $\LUV^2/(4\pi)^2$, confirming that a
gravitational sector emerges at the condensate scale.  This one-loop term is
not what sets the Planck mass, however: $\MPl$ is fixed by the density-feedback
flux of Section~\ref{P7:sec:gravity}, $G_N=G_{\mathrm{src}}^2/(4\pi\vp^6)$
(equation~\eqref{P7:eq:GN}), and~\eqref{P7:eq:1loop} serves as a consistency
check that gravity is induced at the right scale rather than an independent
determination of $\LUV$.

The ultraviolet cutoff is set by the condensate's own $\vp$-tower.  Both $\MPl$
and $\LUV$ lie on the tower $M_n=\Lcond\,\vp^{2n}$ of Paper~VI~\cite{Paper6},
separated by exactly three steps:
\begin{equation}
  \boxed{\LUV \;=\; \frac{\MPl}{\vp^6}\;\approx\;0.0557\,\MPl,}
  \label{P7:eq:MPl}
\end{equation}
the gap $\vp^6=\lambda$ being the same Bogomolny/$S_{\mathrm{eff}}$ suppression
that fixes $G_N$~\eqref{P7:eq:GN} --- a topological relation, not a loop
coincidence.  Numerically
\begin{equation}
...
With $\LUV=\MPl/\vp^6$~\eqref{P7:eq:MPl}, the Planck mass and the cutoff are
locked to the tower spacing $\vp^6$:
\begin{equation}
  \LUV \;=\; \frac{\MPl}{\vp^6}\;\approx\;0.0557\,\MPl,
  \label{P7:eq:LUV_from_MPl}
\end{equation}
the gap $\vp^6=\lambda$ being the Bogomolny parameter that also fixes $G_N$.
Within the bosonic tower $M_n = \Lcond\,\vp^{2n}$ of Paper~VI~\cite{Paper6},
the cutoff lies at the non-integer level
\begin{equation}
  \boxed{n_{\mathrm{UV}}
...
$n\approx76.6$.  The $\vp^6$ gap between them is exact, but neither absolute
level is an integer: the cutoff is locked to the tower spacing without
coinciding with a discrete tower state.
\end{corollary}
...
Since $73.6\notin\mathbb{Z}$, $\LUV$ does not coincide with any discrete tower
level, while the gap $n(\MPl)-n_{\mathrm{UV}}=3$ (i.e.\ $\vp^6$) is exact.
\qed
\end{proof}
...
states as its final theorem (Theorem~7.1 therein).  The $\vp^6$ gap between
$\MPl$ and $\LUV$~\eqref{P7:eq:MPl} is the Bogomolny parameter $\lambda=\vp^6$
--- the same quantity that fixes $G_N$~\eqref{P7:eq:GN}, the condensate energy,
and the $\vp^{-2}$ tower spacing --- so the cutoff sits three tower steps below
the Planck mass for a topological reason.  (The Seeley--DeWitt one-loop
coefficient $4\pi\sqrt{2}\approx17.77$ happens to lie within $1\%$ of
$\vp^6\approx17.94$, but this loop--tower proximity is a numerical coincidence
and plays no role: the cutoff is fixed by the tower relation $\LUV=\MPl/\vp^6$,
and the one-loop term only confirms that a gravitational sector is induced at
the condensate scale.)  Together, Papers~VI and~VII close the tower-to-gravity
connection: Paper~VI establishes the $\vp$-tower from first principles;
Paper~VII locates the induced gravitational sector within it.
\end{remark}
...
\item \textbf{Gravitational sector.} The Planck mass is fixed by the
  ...
  tower steps below it, so no physics above $\LUV$ is needed to determine
  $\MPl$.  The Planck length $\ell_{\mathrm{Pl}}=1/\MPl$ is emergent, not
  a fundamental input.
...
its representative value $\beta\approx0.1$ gives $r_V\approx9.3\times10^6\,\mathrm{AU}$
and $|\gamma_{\mathrm{PPN}}-1|<2.3\times10^{-5}$ from the Vainshtein channel
alone.  The framework's own density-feedback coupling $\bst\approx0.452$
is not restricted to this window: as Remark~\ref{P7:rem:chameleon} shows,
the chameleon screening intrinsic to the density feedback resolves the
Cassini bound at the framework coupling directly, so no separate upper
bound on $\bst$ is required for consistency with Cassini.
\end{remark}

The framework operates at the density-feedback coupling $\bst$ of the
condensate.  The massless-scalar estimate $\oBD=\vp^{12}/(3\bst)$ lies far
below the Cassini bound $\oBD>43\,000$, an apparent tension resolved by
the chameleon screening intrinsic to the density feedback: in a dense
environment $\oBD^{\mathrm{eff}}=\oBD(1+\bst\rho)$, and at Solar-System
density $\bst\rho_\odot\sim10^{6}$, giving $\oBD^{\mathrm{eff}}\sim10^{8}
\gg43\,000$ --- a margin of $\sim10^{3}$--$10^{4}$ at the framework's own
coupling, with no fine-tuning to small $\bst$.  The Vainshtein radius sets
the scale below which this screening becomes nonlinear.
...
\item The Brans--Dicke parameter is $\oBD=\vp^{12}/(3\beta)$,
  ...
  $\lam=\vp^6$ of the $Q=2$ condensate (Paper~I~\cite{Paper1}).
...
where $\vp^{20}$ carries the topological exponent $2Q_{\mathrm{condensate}}=20$
and $\vp^{12}=\lam^2$ is the squared Bogomolny parameter $\lam=\vp^6$.
...
The exponent $\vp^{-8/3}=\vp^{(12-20)/3}$ carries the $\vp^{20}$ of the
\emph{Jordan-frame} scalar mass~\eqref{P7:eq:mxi} (step~\emph{(ii)}).  The
physical Einstein-frame mass is $\vp$-power-free ($m_\xi\sim H_0$: the strong
nonminimal coupling $1+3\vp^7$ cancels the $\vp^{20}$ curvature).  Replacing
the Jordan mass by the physical one already shifts the exponent --- to
$r_V\propto\vp^4$ with $\oBD$ held fixed, and further once $\oBD$ is itself
expressed in the Einstein frame --- so the golden-ratio scaling of $r_V$ is a
Jordan-frame quantity rather than a frame-invariant statement, and the
``topological invariant'' reading holds only in that frame.  This does not
affect the Cassini resolution, which is secured independently and
frame-robustly by the chameleon channel of Remark~\ref{P7:rem:chameleon},
whose density-dependent coupling $\oBD^{\mathrm{eff}}=\oBD(1+\bst\rho)$ is
...
The Planck mass and the cutoff are locked to the tower spacing,
$\LUV=\MPl/\vp^6$~\eqref{P7:eq:MPl}, so the two scales are not
...
$(\LUV/\MPl)^2 = 1/\vp^{12}$ (from~\eqref{P7:eq:MPl}):
\[
  \mathcal{P}_s
...
\end{equation}
The exponent is $\vp^6\rinfty = \vp^7 c/\bst$, where $c=\rho/\kappa$ is
the normalisation between the $k$-essence field and the Faddeev--Niemi
gradient density (Remark~\ref{P7:rem:rho_extension}, $\beta=\bst/c$).  Two
clean framework values bracket it: the canonical normalisation of the
kinetic term ($\sqrt{\vp^6}=\vp^3$) fixes the leading $c=\vp^3$, giving
$\sim10^{-118}\,\MPl^4$, while the topological coupling $\beta=1/Q$ gives
$c=\vp^3+2p_0^2\approx4.52$ and $\sim10^{-126}\,\MPl^4$.  The correction
unit is the square of the vacuum Born weight
$p_0 = 1/(1+\bst\rinfty) = 1/\vp^2$ (the same weight that carries
$\aem^{-1}=k|2I|\,p_0$, Paper~III~\cite{Paper3}), $p_0^2=1/\vp^4$, and the
observed value sits at the midpoint,
\begin{equation}
  c \;=\; \vp^3 + p_0^2 \;=\; \vp^3\Bigl(1+\frac{1}{\vp^7}\Bigr),
  \label{P7:eq:cc_c}
\end{equation}
giving $\approx10^{-122}\,\MPl^4$ --- the geometric mean of the canonical
and topological brackets --- in agreement with observation.  This is the
framework's bracket-then-refine structure: a derived leading $\vp$-power,
bracketed by the topological value, with a single anyonic correction
locating the observed within, as in the Chern--Simons refinement of the
fine-structure formula (Paper~IV~\cite{Paper4}).  The correction is
$p_0^{Q_H}$ --- the vacuum weight raised to the Hopf charge of the
condensate.  With $Q_H=2$ (the icosahedral Hopfion of Paper~I~\cite{Paper1}) this is
$p_0^2$; a standard $Q_H=1$ vacuum would give $p_0$ and
$\sim10^{-129}\,\MPl^4$, so the observed value is further evidence for the
charge-two vacuum.  ``Two insertions'' is not an abstract choice --- it is
one vacuum-weight insertion per unit of Hopf charge: the $k$-essence
field, dressed by the vacuum, threads through the condensate's linked
fibres, and each link is one monodromy inserting a single factor of
$p_0$.  The vacuum weight is itself proved modular data:
$p_0 = 1/V^{*2}$, where $V^* = S_{0,1/2}/S_{0,0} = \vp$ is the WZW virial
value proved by three-fermion Witten bosonization
(Paper~III~\cite{Paper3}), the same value that fixes the Linking Scale and
$\aem^{-1}$.  The correction $p_0^{Q_H}=V^{*-2Q_H}$ is thus the same
icosahedral WZW monodromy that carries the fine-structure constant,
weighted here by the Hopf linking number rather than by $k|2I|$.
...
(Paper~I~\cite{Paper1}), canonically normalises via
$d\chi/d\rho=[\vp^6(1+\bst\kappa)]^{-1/2}$; in the ultraviolet limit
$\bst\kappa\to0$ this is $\vp^{-3}$, so $c_{\mathrm{tree}}=\vp^3=\sqrt{\lam}$.

\emph{Vacuum weight.} The $k=3$ modular $S$-matrix satisfies
$S_{1/2,1/2}=S_{0,0}$ (Paper~II~\cite{Paper2}), so the vacuum monodromy
scalar is $p_0=S_{1/2,1/2}S_{0,0}/S_{0,1/2}^2=(S_{0,0}/S_{0,1/2})^2=1/V^{*2}$,
with $V^*=S_{0,1/2}/S_{0,0}=\vp$ proved for all $R_0$ by three-fermion
Witten bosonization (Paper~III~\cite{Paper3}).  Hence $p_0=1/\vp^2$.

\emph{Linking count.} The vacuum-dressing correction to $c$ is the field
self-energy --- the normalised Wilson-loop expectation of the
spin-$\tfrac12$ fluctuation in the $Q_H$-Hopfion background.  By the
Chern--Simons evaluation in the condensate WZW theory
(Paper~III~\cite{Paper3}), an unknot of colour $j$ gives the quantum
dimension $S_{0j}/S_{0,0}$ and two spin-$\tfrac12$ loops of linking number
$n$ give $M_{1/2,1/2}^{\,n}$, where
$M_{1/2,1/2}=S_{1/2,1/2}S_{0,0}/S_{0,1/2}^2=p_0$ is the monodromy scalar.
The Hopf charge is the linking number of the field's fibre preimages
(Paper~I~\cite{Paper1}), so the self-energy loop links the condensate flux
$Q_H$ times and $c_{\mathrm{vac}}=p_0^{\,Q_H}$.

\emph{Assembly.} The wave-function renormalisation is
$Z=1+c_{\mathrm{vac}}/c_{\mathrm{tree}}$, so
$c=c_{\mathrm{tree}}+c_{\mathrm{vac}}=\vp^3+p_0^{\,Q_H}$.  At $Q_H=2$,
$c=\vp^3+\vp^{-4}=4.382$, whence $\beta=\bst/c=0.1032$,
$\rinfty=\vp/\beta=15.68$, and
$\Lobs=\Lambda_0 e^{-\vp^6\rinfty}=10^{-122.2}\,\MPl^4$.
\end{proof}

\begin{remark}[Status of the linking step]
\label{P7:rem:cc_norm_status}
The tree normalisation, the vacuum weight $p_0=1/V^{*2}$ (with $V^*=\vp$
proved by bosonization), and the self-energy --- evaluated above as a
Chern--Simons Wilson loop with the same $S$-matrix that fixes
$\aem^{-1}=k|2I|\,p_0$ --- are all computed.  The one topological input is
that the self-energy loop's linking with the condensate flux equals the
Hopf charge, which is the definition of the Hopf invariant (linking of the
field's fibre preimages, Paper~I~\cite{Paper1}).  Crucially, this is the
\emph{geometric} Hopf charge $Q_H=2$, distinct from the \emph{algebraic}
charge $Q=2\,\mathrm{rank}(G_2)+(\text{generation sum})=10$ that organises
the geometric quantisation (Paper~X~\cite{Paper10}).  The $\rho$-fluctuation
propagator of the quantised background (Paper~X~\cite{Paper10}) supplies the
loop; the braiding weight and the geometric linking are fixed by Papers~I
and~III.
\end{remark}
...
\begin{equation}
  ...
  \label{P7:eq:mxi}
\end{equation}
in the Jordan frame.  The physical mass governing the cosmological (thawing)
dynamics is the Einstein-frame value, obtained after the conformal rescaling
by $F(\rinfty)$~\eqref{P7:eq:Feff_derived}: because that nonminimal coupling
is strong ($1+3\vp^7\approx88$), the conformal kinetic normalisation cancels
the $\vp^{20}$ curvature, leaving the $\vp$-power-free Hubble-scale result
$m_\xi\sim\sqrt{\Lobs}/\MPl\approx H_0\approx10^{-33}\,\mathrm{eV}$.
The Vainshtein/screening analysis of Section~\ref{P7:sec:cassini} uses the
Jordan-frame expression~\eqref{P7:eq:mxi} directly; the resulting PPN
parameter resolves Cassini, and the outcome is unchanged if the physical
mass is used, since $r_V$ remains far larger than the solar system either way.

\begin{remark}[Dark-energy sound speed]
\label{P7:rem:cs_de}
The sound speed of the dark-energy scalar's perturbations at the attractor
is fixed with no free parameters.  With $K(\rho)=1/[\vp^6(1+\beta\rho)]$
(equation~\eqref{P7:eq:K_attractor}), $\beta\rinfty=\vp$ and the golden-ratio
identity $1+\vp=\vp^2$, the $k$-essence sound speed is
\begin{equation}
  ...
  \label{P7:eq:cs_de}
\end{equation}
This is an order-unity (relativistic) speed: dark-energy perturbations are
smooth on sub-horizon scales and do not cluster, as required of a
$\Lambda$-like component.  It is distinct from the $O(\beta^0)$ acoustic-metric
speed $c_s^2=1$ of Section~\ref{P7:sec:acoustic_schwarz}, which sets the
propagation cone of $\xi$-fluctuations on the black-hole background: the two
are the same $k$-essence sound speed evaluated at low density and at the
...
%══════════════════════════════════════════════════════════════════════════════
\section{Dark Matter: Elastic Strain of the Condensate Director}
\label{P7:sec:dark_matter}
%══════════════════════════════════════════════════════════════════════════════

The framework introduces no dark-matter particle. Instead, the condensate
scalar field $\rho$ itself acts as the dark-matter component through its
anisotropic, nematic-like medium.  Its directional suppression
$S_{\mathrm{eff}}=\sin^4\!\theta/[\vp^6(1+\bst\rho)]$ (Axiom~A2) is not
merely analogous to a semi-Dirac semimetal: it \emph{is} the angular
structure of a semi-Dirac dispersion.  Writing the two-band condensate
fluctuation Hamiltonian with its quadratic (heavy) axis perpendicular to
the preferred direction $\hat{n}=\hat{e}_r$ and its linear (light) axis
along it,
$H=A\,k_\perp^2\,\sigma_x+v\,k_\parallel\,\sigma_y$ with
$k_\parallel=k\cos\theta$ and $k_\perp=k\sin\theta$, gives
\begin{equation}
...
$S_{\mathrm{eff}}$: the transverse quadratic band
$k_\perp^2\propto\sin^2\!\theta$, squared in the energy, so
$\sin^4=(\sin^2)^2$.  The anisotropy is intrinsic to the band structure,
not imposed --- motion is easy (linear, light) along $\hat{e}_r$ and hard
(quadratic, heavy) perpendicular to it --- and it is the same
$\sin^4=(\sin^2)^2$ structure that, through the pentagon $k+2=5$, fixes
the three fermion generations.  The dark-matter physics lives in the
orientation (director) field $\hat{n}$ (Axiom~A1), the medium's soft
mode, not in the scalar magnitude $\rho$.

%──────────────────────────────────────────────────────────────────────────────
\subsection{Dark matter as director strain}
\label{P7:sec:director_strain}
%──────────────────────────────────────────────────────────────────────────────

Visible matter --- baryons, the excitations of the condensate --- imposes
orientation boundary conditions on the director.  The director distorts
in response, and the Frank elastic energy
\begin{equation}
  F=\tfrac12\int\bigl[K_{\mathrm{splay}}(\nabla\!\cdot\hat{n})^2
    +K_{\mathrm{bend}}(\hat{n}\times(\nabla\times\hat{n}))^2\bigr]\,d^3x
  \label{P7:eq:frank}
\end{equation}
of the distortion is real energy: it gravitates ($E=mc^2$) but carries
no electromagnetic coupling, so it is invisible.  Dark matter is not a
substance but the medium's elastic response to visible matter.  Being
sourced by the baryons, the strain tracks them automatically,
reproducing the tight radial-acceleration relation that particle
dark-matter halos must be tuned to match.

%──────────────────────────────────────────────────────────────────────────────
\subsection{Spontaneous radial texture: the splay instability}
\label{P7:sec:splay_instability}
%──────────────────────────────────────────────────────────────────────────────

The two director stiffnesses of the semi-Dirac medium have opposite
signs: bend is stable, $K_{\mathrm{bend}}>0$, while splay is unstable,
\begin{equation}
  \boxed{K_{\mathrm{splay}}<0.}
\end{equation}
The uniform director is therefore unstable and spontaneously develops a
radial splay texture: a disclination configuration in which the
director cannot remain uniformly radial and must wind.  This is the
"less-radial" frustration region, and it forms without external tuning
--- the self-organised dark-matter configuration.

%──────────────────────────────────────────────────────────────────────────────
\subsection{Cold director mode: pressureless clustering}
\label{P7:sec:cold_director}
%──────────────────────────────────────────────────────────────────────────────

Whether the strain behaves as cold dark matter is settled by the dynamics
of the director, not the statics.  The orientation Goldstone mode has speed
\begin{equation}
  c_{\mathrm{dir}}^2 \;=\; \frac{K}{\chi},
  \label{P7:eq:cdir}
\end{equation}
where $\chi$ is the static susceptibility of the generator of director
rotations.  In the semi-Dirac medium the rotational susceptibility diverges
faster with the ultraviolet scale than the Frank stiffness $K$, so the
director is \emph{heavy}: $c_{\mathrm{dir}}\to0$.  The director mode is
cold.  This is the decisive contrast with the scalar-magnitude
perturbation, whose sound speed $c_s=1/\vp$ (Remark~\ref{P7:rem:cs_de}) is
relativistic --- that mode belongs to the dark-energy sector, not to dark
matter.

Being cold and pressureless, the director strain has no Jeans obstruction
and clusters on all cosmological and galactic scales.  In the splay
channel, $K_{\mathrm{splay}}<0$ turns~\eqref{P7:eq:cdir} into an
instability, $c_{\mathrm{dir}}^2<0$ --- an imaginary frequency, the
dynamical form of the self-organisation of
Section~\ref{P7:sec:splay_instability}.  The static strain gravitates while
the slow mode supplies no pressure support; the two are decoupled, and the
component clusters as cold dark matter wherever it is present.

%──────────────────────────────────────────────────────────────────────────────
\subsection{Flat rotation curves from the disclination}
\label{P7:sec:flat_curves}
%──────────────────────────────────────────────────────────────────────────────
...
\begin{equation}
  ...
  \label{P7:eq:Menc}
\end{equation}
$M_{\mathrm{enc}}\propto r$ is exactly $v^2=GM_{\mathrm{enc}}/r=\text{const}$
--- flat rotation curves, with no fitted halo.  A smooth, defect-free
director gives the fast-falling monopole (Keplerian); the flat curve is
the signature of the splay-driven disclination.

\begin{remark}[Director strain as a heavy elastic solid]
\label{P7:rem:elastic_solid}
Because the stiffnesses have standard Frank form, the director strain is
an elastic medium with genuine shear rigidity, not a collisionless gas.
It is \emph{heavy}: the static rigidity is large while the shear-wave
...
acts \emph{dynamically} depends on the cutoff.  Because
$c_{\mathrm{dir}}$ falls faster than the stiffness grows, at a very large
ultraviolet cutoff the shear waves are too slow to respond on galactic
timescales: the strain is then a \emph{static}, cold, pressureless
gravitating field --- phenomenologically indistinguishable from standard
collisionless cold dark matter.  Only if the relevant cutoff sits near
the acceleration scale (Section~\ref{P7:sec:accel_scale}), where
$c_{\mathrm{dir}}$ exceeds galactic rotation velocities, does the elastic
response act on rotation timescales; there the medium clusters coherently
and tends to suppress the small-scale substructure a collisionless halo
develops, and the splay-unstable channel ($K_{\mathrm{splay}}<0$,
Section~\ref{P7:sec:splay_instability}) renders disk warps, corrugations
and rings intrinsic elastic modes rather than tidal accidents.  Robust
are the signs: shear rigidity, the coldness $c_{\mathrm{dir}}\to0$, and
the splay instability.  Whether the distinguishing elastic dynamics
survive at all, and on what scales, is set by the same condensate cutoff
as the acceleration scale and is not fixed here.  The overall
\emph{magnitude}, by contrast, is condensate-derived: the Frank constant
follows from the condensate energy density
$\rho_{\mathrm{cond}}\sim10\Lcond^4$ (fixed by $\TCMB$) and the ultralight
director mass $m_\xi\sim H_0$ (fixed by the dark-energy sector), and
reproduces the value required by galactic rotation curves to within about
an order of magnitude with no galactic input.
\end{remark}

\begin{remark}[Director strain as a polaron: the effective inertial mass]
\label{P7:rem:polaron}
The strain of Section~\ref{P7:sec:director_strain}, sourced by and
comoving with the baryons, dresses each baryon in the manner of a
polaron---a particle accompanied by the medium distortion it induces.
The quantity that gravitates and acts dynamically is then the
\emph{effective inertial mass} of the dressed baryon, read off from the
momentum carried by the comoving distortion.  With the director dynamics
$\chi\,\partial_t^2\hat{n}=K\nabla^2\hat{n}$ (so
$c_{\mathrm{dir}}^2=K/\chi$, equation~\eqref{P7:eq:cdir}) and a distortion
$\hat{n}(\mathbf{x}-\mathbf{V}t)$ comoving with the source, the field
momentum is $P_i=M_{ij}V_j$ with
\begin{equation}
  M_{ij}=\chi\!\int\!\partial_i\hat{n}\cdot\partial_j\hat{n}\,d^3x,
  \qquad
  m_{\mathrm{eff}}=\tfrac{\chi}{3}\!\int\!|\nabla\hat{n}|^2\,d^3x .
  \label{P7:eq:meff}
\end{equation}
This mass is positive by construction: $\chi$ is a susceptibility and the
integrand is a sum of squares, so \emph{the sign of the Frank constants
does not enter}.  The splay instability $K_{\mathrm{splay}}<0$
(Section~\ref{P7:sec:splay_instability}) drives the \emph{formation} of
the texture---it makes $c_{\mathrm{dir}}^2<0$ in the splay channel---but
the inertial mass depends on $\chi>0$ and the gradient integral, not on
the sign of the elastic energy; formation and gravitating mass are
distinct.  For the disclination halo $|\nabla\hat{n}|\sim1/r$
(Section~\ref{P7:sec:flat_curves}), $m_{\mathrm{eff}}(r)\propto r$,
reproducing $M_{\mathrm{enc}}\propto r$ with a positive mass, and the
medium is \emph{heavy} because $\chi=K/c_{\mathrm{dir}}^2$ grows as
$c_{\mathrm{dir}}\to0$ (Remark~\ref{P7:rem:elastic_solid}).

The same dressing acts dynamically: a moving baryon drags its distortion
cloud, so the halo produces the gravitational dynamical friction of any
massive halo.  Whether a further \emph{elastic} drag---from the finite
relaxation of the lagging cloud---is present depends on the rotational
viscosity of the director and is not fixed here; if present it is
velocity-dependent with a scaling distinct from the Chandrasekhar
$1/v^2$, a signature by which the elastic halo could in principle be
distinguished from a collisionless one.
\end{remark}

\begin{remark}[What the dark matter is: coherent deformation without a knot]
\label{P7:rem:cdm_ontology}
The dark sector is four states of the one condensate.  Baryonic matter
is a Hopfion, carrying a topological Hopf charge, localised and 
electromagnetically coupled.  Dark matter is the coherent
\emph{deformation} of the surrounding director carrying no Hopf charge
---a texture of the medium, not the knot itself---that such a knot induces
(Section~\ref{P7:sec:director_strain}), and which persists as a real,
gravitating strain \emph{independently} of the knot that sourced it:
because the director mode is cold ($c_{\mathrm{dir}}\to0$,
equation~\eqref{P7:eq:cdir}), a deformation relaxes only over
cosmological times and survives after the sourcing baryon has moved on.
It is stress without heat---a coherent geometric strain, as opposed to
the \emph{incoherent} thermal excitations of the condensate, which
redshift as radiation, and to the frozen vacuum, which is the
cosmological constant.  Dark matter here is therefore not a particle but
the cold, coherent, electromagnetically neutral deformation field of the
fabric; its gravitating mass is the effective inertial mass of that
comoving deformation (Remark~\ref{P7:rem:polaron}).  The same coherent
deformation may in principle exist without a present-day source---a
frozen relic strain of the fabric---which is the natural (and open)
candidate for any cold component required before structure forms.

Throughout the series the Hopf charge of a sector is counted
\emph{relative to} this condensate ground state, whose own configuration
carries winding (the $Q=2$ vacuum of Paper~I~\cite{Paper1}); it is this
relative charge that distinguishes the baryonic knot, which carries it,
from the director deformation, which carries none.  The excitation ladder
of the later particle-spectrum papers ($Q_H=1,2,3$ for the neutrino,
charged-lepton, and baryon sectors) is defined on the same relative
footing.
\end{remark}

%──────────────────────────────────────────────────────────────────────────────
\subsection{The acceleration scale}
\label{P7:sec:accel_scale}
%──────────────────────────────────────────────────────────────────────────────

The strain magnitude is set by the physical ultraviolet cutoff of the
semi-Dirac dispersion --- the condensate bandwidth, which in this
framework is the condensate scale $m_\xi\sim H_0$
(Section~\ref{P7:sec:de}).  The acceleration at which the strain
contribution overtakes the Newtonian term is therefore of order $cH_0$,
coinciding with the empirical MOND scale $a_0\approx cH_0/(2\pi)$ and
tying the dark-matter and dark-energy scales to the same condensate.
The coefficient is fixed by the ratio of the cutoff $m_\xi$ to the
semi-Dirac crossover scale.  Reproducing the empirical
$a_0\approx cH_0/(2\pi)$ requires this ratio to be of order $\vp^6$ ---
the same normalisation that appears in the anisotropy $S_{\mathrm{eff}}$
itself --- with the light-axis and heavy-axis identifications of the
dispersion bracketing the observed value within a factor of order unity.
This is a framework-native scale rather than a free coefficient, but the
precise value is not derived here.

%──────────────────────────────────────────────────────────────────────────────
\subsection{Screening of the gravitational coupling in the halo}
\label{P7:sec:halo_screening}
%──────────────────────────────────────────────────────────────────────────────
...
the flat curves of Section~\ref{P7:sec:flat_curves} are a director-strain
effect, not a variation of $G_{\rm eff}$.
\end{proposition}
...
\begin{remark}[Colliding clusters and the lensing--gas offset]
\label{P7:rem:bullet}
The archetypal test of collisionless dark matter is a cluster merger
such as the Bullet cluster~\cite{Clowe2006}, in which the member
galaxies pass through nearly collisionlessly while the intracluster
gas---the dominant baryonic component---is shocked and retarded, so
that weak lensing locates the gravitating mass on the galaxies,
offset from the gas.  In the present framework this offset is the
expected outcome rather than an anomaly.  The director strain is
sourced by the coherent orientation boundary conditions a bound
galaxy imposes on the medium (Section~\ref{P7:sec:director_strain});
each galaxy carries its own splay halo, comoving with it, which is
transported through the merger and so behaves collisionlessly in the
encounter despite the shear rigidity of the medium
(Remark~\ref{P7:rem:elastic_solid}).  The shocked gas, by contrast,
is turbulent and orientationally incoherent: it offers no coherent
seed for the splay disclination of
Section~\ref{P7:sec:splay_instability} and sources negligible strain.
The invisible mass therefore tracks the galaxies rather than the
dominant gas, with no collisionless particle---the conclusion the
observation is usually taken to compel, reached here from the
elasticity of the medium.

The \emph{amplitude} at cluster scales is a separate, quantitative
question.  A galaxy's strain, normalised to its rotation curve, is an
isothermal halo $M_{\mathrm{strain}}\propto v^{2}r$ with $v$ the
galactic circular speed; summed independently over the members it
underestimates the cluster lensing mass by a factor of order unity.
The deficit closes to the extent that the inter-galaxy strain
responds to the collective potential of the member system rather than
to each galaxy in isolation, since an isothermal halo at the cluster
velocity dispersion~$\sigma$ reproduces the virial mass identically,
$M\propto\sigma^{2}r$, leaving $(\sigma/v)^{2}$ of headroom over the
shortfall.  The collectivity of the director response is set by the
same elasticity as the single-galaxy halo and, like the acceleration
scale of Section~\ref{P7:sec:accel_scale}, is not fixed here; a
residual cluster-scale enhancement of order unity, common to elastic
and modified-gravity accounts, remains open.
\end{remark}

\begin{remark}[Assembly-seeded disclinations: a baryon-independent cluster channel]
\label{P7:rem:defect_network}
The residual enhancement left open in Remark~\ref{P7:rem:bullet} admits a
mechanism with no counterpart in a force law slaved to the local baryonic
acceleration.  Cluster assembly is hierarchical and turbulent: repeated
mergers stir the director on the subcluster scale, and where domains of
independently established orientation meet they trap disclination lines, in
the manner of the Kibble--Zurek mechanism.  Because the director is cold
($c_{\mathrm{dir}}\to0$, equation~\eqref{P7:eq:cdir}), such trapped defects
cannot heal on the dynamical time; the merger leaves behind a frozen
disclination network.  This network sources strain in its own right, seeded
by the assembly history rather than by any single galaxy's boundary
conditions, and is therefore additional to the baryon-tracking strain of
Section~\ref{P7:sec:director_strain}.

With line density of order one defect per coherence area
$\xi_{\mathrm{turb}}^2$, the network mass scales as
$M_{\mathrm{def}}/M_{\mathrm{strain}}\sim(R_{\mathrm{cl}}/\xi_{\mathrm{net}})^2$,
with $\xi_{\mathrm{net}}$ the frozen-network spacing and
$\xi_{\mathrm{turb}}$ the merger injection scale.  When the network freezes
at the injection scale, $\xi_{\mathrm{net}}\sim\xi_{\mathrm{turb}}$, and for
subcluster mergers ($\xi_{\mathrm{turb}}\sim\tfrac12 R_{\mathrm{cl}}$) this
is of the order required to close the cluster deficit --- recovered from the
assembly scale, with no free normalisation beyond the $a_0$ anchor that
fixes $M_{\mathrm{strain}}$.  A defect-network relaxation with the correct
inertial (heavy-director) dynamics confirms the scaling: the network freezes
at $\xi_{\mathrm{net}}\sim\xi_{\mathrm{turb}}$ in the $c_{\mathrm{dir}}\to0$
limit and coarsens away otherwise, with a coarsening time that diverges as
$c_{\mathrm{dir}}\to0$.

Whether the network freezes or coarsens is set by the rate at which the
trapped defects \emph{heal}, which is distinct from the director signal
speed $c_{\mathrm{dir}}$.  A topological defect cannot be removed by smooth
reconfiguration, however fast: it heals only by annihilating against an
anti-defect or by melting its core.  Its lifetime is therefore governed by
the defect \emph{mobility} --- the line tension driving annihilation set
against the rotational viscosity resisting it (the same viscosity behind the
elastic drag of Remark~\ref{P7:rem:polaron}, likewise not fixed here),
together with the heavy inertia of the cold director --- and not by how fast
the medium can reorient smoothly.  This separation protects the network
against the concern that the shared-orientation geometry underlying the
framework's nonlocality permits fast \emph{global} reconfiguration of the
medium: that smooth mode carries no topological charge and cannot annihilate
a disclination, so it heals smooth strain but not the trapped network.  The
survival condition is thus that the annihilation rate --- not the
reconfiguration speed --- be slow compared with cluster assembly, met in the
same heavy, cold-director limit ($c_{\mathrm{dir}}\to0$,
Section~\ref{P7:sec:cold_director}) the sector already requires for
clustering, and contingent on the same cutoff-set quantities, not derived
here.  Whether the residual healing outpaces or lags assembly under cosmic
expansion is the open point.

This channel carries a distinguishing signature.  Because the frozen network
records the assembly history and only slowly coarsens, the enhancement
depends on the dynamical state of the cluster: recently merged or disturbed
clusters, freshly re-stirred, carry a denser network and a larger mass
excess than relaxed clusters of the same baryonic content.  A force law tied
to the local baryonic acceleration predicts no such dependence, and a
collisionless halo ties the excess to the halo rather than to the merger
state.
\end{remark}

%──────────────────────────────────────────────────────────────────────────────
\subsection{Structure formation and the baryon-acoustic scale}
\label{P7:sec:bao}
%──────────────────────────────────────────────────────────────────────────────

Because the director strain is pressureless, carries no independent sound
speed, and is sourced by and comoving with the baryons, it adds no
acoustic feature of its own: it shifts no acoustic scale.  This no-shift
property is firm.  It does not, on its own,
fix the \emph{absolute} acoustic scale.  The standard
$r_s\approx147\,\mathrm{Mpc}$ is set by the pre-recombination matter budget
(the epoch of matter--radiation equality and the baryon loading), so
recovering it requires a cold-matter component present at recombination.
Whether this sector supplies one is tied to the ordering epoch of
Section~\ref{P7:sec:dm_ic} and is not settled here: the self-organised
texture orders only at $z\sim0$ and is absent at recombination, while the
recombination-era presence of the adiabatic baryon-sourced strain is left
open.  The firm prediction of the sector is therefore the \emph{absence of
a shift}; recovery of the standard scale is contingent on that ordering
epoch, not guaranteed by the no-shift property alone.

%──────────────────────────────────────────────────────────────────────────────
\subsection{Initial conditions from reheating}
\label{P7:sec:dm_ic}
%──────────────────────────────────────────────────────────────────────────────
...
populates radiation and establishes the condensate --- the nematic medium
in which the director then orders.  The dark-matter initial conditions
follow from the director sector.  The cold, baryon-sourced strain of
Section~\ref{P7:sec:cold_director} inherits the adiabatic spectrum
$\mathcal{P}_s=2.10\times10^{-9}$ (Proposition~\ref{P7:prop:Treh}) through
the baryons it tracks, with no additional free parameter.  The splay
self-organisation of Section~\ref{P7:sec:splay_instability} adds a
...
ultralight ($m_\xi\sim H_0$, Section~\ref{P7:sec:de}), it is Hubble-frozen
until $H\sim m_\xi$ and orders only at $z\sim0$; the texture therefore
forms in the present epoch, is absent at recombination, and carries
negligible cosmic-microwave-background isocurvature, safely below the
Planck bound.  The same lateness fixes what this texture component is: a
late-time, galactic-scale contribution, not a source of dark matter at
recombination.  Whether the adiabatic baryon-sourced strain instead
operates at recombination depends on the epoch at which the nematic medium
orders --- set by the same condensate cutoff and not fixed here; in either
case the strain shifts no acoustic scale.  The earlier proposal that the
condensate is itself the inflaton, giving $\varepsilon\approx\beta/\vp^6$,
is excluded by the Higgs/Starobinsky value
$\varepsilon=2/N_e^2\approx0.001$ (Section~\ref{P7:sec:inflation}).
\end{remark}

%──────────────────────────────────────────────────────────────────────────────
\subsection{Status and inputs}
\label{P7:sec:dm_status}
%──────────────────────────────────────────────────────────────────────────────

The mechanism --- invisible director strain, splay-unstable
self-organisation, a cold orientation mode, and a disclination giving
$M_{\mathrm{enc}}\propto r$ and flat curves --- is established at the level
of the sign of $K_{\mathrm{splay}}$, the standard Frank form, and the
heaviness of the director ($c_{\mathrm{dir}}\to0$, pressureless
clustering).  What is \emph{not} fixed here is set by the physical
condensate cutoff: the coefficient in $a_0$ (the ratio of the cutoff to
the semi-Dirac crossover, of order $\vp^6$), and the epoch at which the
nematic medium orders --- which decides whether the sector contributes
recombination-era cold dark matter or only the late-time, galactic-scale
component established here.  The self-organised texture is
isocurvature-safe by its lateness (Section~\ref{P7:sec:dm_ic}).  The
present-day dark-matter density $\Omega_{\mathrm{DM}}\approx0.26$ is taken
as an external observed measurement and is not derived here; unlike the
mass-chain inputs $\TCMB$ and $m_e$, it enters no derivation of this paper.
...
\begin{remark}[Oblate spheroidal approximation]
\label{P7:rem:oblate}
The ansatz $1/\sqrt{r^2+a^2\cos^2\!\theta}$ is the generating
...
Cassini $r_V\propto\vp^{-8/3}$ golden-ratio scaling from $Q$ & \textcolor{darkgreen}{Proved (Cor.~\ref{P7:cor:vainshtein_topology})}, Jordan-frame (Rem.~\ref{P7:rem:rV_frame}) & --- & Cassini secured by chameleon \\
...
Dark matter as director strain: splay instability, disclination gives flat curves & \textcolor{darkgreen}{Mechanism established (Sec.~\ref{P7:sec:dark_matter}); magnitude cutoff-set} & $M_{\rm enc}\propto r$ & --- \\
$G_{\rm eff}(r)/G_N\leq1+6\times10^{-5}$ inside MW halo & \textcolor{darkgreen}{Proved (Prop.~\ref{P7:prop:Geff})} & exact & --- \\
Baryon-acoustic scale: no shift (director strain pressureless, comoving with baryons) & \textcolor{darkgreen}{Predicted (Sec.~\ref{P7:sec:bao})} & no shift & absolute $r_s$ contingent on ordering epoch \\[4pt]
...
CC $\Lambda_0 e^{-\vp^7/\beta}$: $c=\vp^3{+}p_0^2$, bracket $[\vp^3,\vp^3{+}2p_0^2]$ & \textcolor{darkgreen}{Predicted (leading $\vp^3$ + anyonic refinement)} & $\sim10^{-122}$ & matches obs. \\
...
\item \textbf{[Open] Dark matter: strain magnitude and the $a_0$ coefficient.}
  Section~\ref{P7:sec:dark_matter} identifies dark matter as the Frank
  elastic strain of the condensate director: the splay instability
  $K_{\mathrm{splay}}<0$ is established, and the resulting disclination
  gives $M_{\mathrm{enc}}(r)\propto r$ and flat rotation curves with no
  fitted halo (Section~\ref{P7:sec:flat_curves}).
  $G_{\rm eff}(r)=G_N$ to $6\times10^{-5}$ at rotation-curve radii
  (Proposition~\ref{P7:prop:Geff}), so the flat curves are a
  director-strain effect, not a variation of the gravitational coupling.
  Open: the strain magnitude, and hence the precise coefficient in the
  MOND-scale acceleration $a_0\approx cH_0/(2\pi)$
  (Section~\ref{P7:sec:accel_scale}), require the condensate's
  ultraviolet cutoff regularisation for a quantitative value.
  $\Omega_{\mathrm{DM}}\approx0.26$ remains an input, not a derived
  quantity.  The director strain tracks the baryons and makes no
  shifted-BAO prediction (Section~\ref{P7:sec:bao}).
...
(the profile self-consistency conjecture; Papers~II--III~\cite{Paper2,Paper3}).}
  ...
  \textbf{[Resolved in Paper~XII~\cite{Paper12}] Layer 2: $\vp^9\sqrt{\Ja\Jfour} = Q$}.}
...
exterior, a one-loop induced Einstein--Hilbert term confirming a
gravitational sector at the condensate scale (the Planck mass itself
fixed by the flux), and the
cosmological-constant suppression $\Lambda_0 e^{-\vp^7/\beta}$, with a canonical leading normalisation $c=\vp^3$ refined by an anyonic $p_0^2$ correction, predicting $\approx10^{-122}\MPl^4$ in agreement with the observed $\Lobs$.
...
The Planck mass is fixed by the density-feedback flux, and the condensate
ultraviolet cutoff sits three $\vp$-tower steps below it,
$\LUV=\MPl/\vp^6\approx0.0557\,\MPl$, at the non-integer level $n_{\rm UV}\approx73.6$
--- the $\vp^6$ gap being the Bogomolny parameter, locked to the tower spacing
without coinciding with a discrete state (Corollary~\ref{P7:cor:luv_tower};
connects to Paper~VI~\cite{Paper6}).
...
gives $r_V$ a golden-ratio scaling $\vp^{-8/3}$
(Corollary~\ref{P7:cor:vainshtein_topology}); this scaling is a Jordan-frame
quantity (Remark~\ref{P7:rem:rV_frame}), while the Cassini resolution itself is
secured frame-robustly by the chameleon channel
(Remark~\ref{P7:rem:chameleon}).
...
Dark matter is identified as the invisible Frank elastic strain of the
condensate director, not as a new particle or as the scalar $\rho$
itself.  The two director stiffnesses of the semi-Dirac medium have
opposite signs, with $K_{\mathrm{splay}}<0$: the uniform director is
splay-unstable and self-organises into a radial disclination.  The
resulting enclosed strain mass grows linearly, $M_{\mathrm{enc}}(r)
\propto r$ (equation~\eqref{P7:eq:Menc}), giving flat rotation curves
with no fitted halo, while $G_{\rm eff}$ inside the Vainshtein sphere of
a Milky-Way galaxy remains $G_N$ to $\lesssim6\times10^{-5}$
(Proposition~\ref{P7:prop:Geff}), so the flat curves are not a
gravitational-coupling effect.  Being sourced by, and comoving with,
the baryons, the strain tracks them automatically, reproducing the
radial-acceleration relation and introducing no acoustic feature of its
own, so the sector shifts no acoustic scale; recovery
of the standard $r_s\approx147\,\mathrm{Mpc}$ is contingent on whether the
strain supplies a cold-matter component at recombination
(Section~\ref{P7:sec:dm_ic}), which is not settled here.
The strain magnitude, and the coefficient of the resulting MOND-scale
acceleration $a_0\approx cH_0/(2\pi)$, are set by the condensate's
ultraviolet cutoff and require its regularisation for a quantitative
value; $\Omega_{\mathrm{DM}}\approx0.26$ remains an input.
...
the physical one.
Two observational questions remain open: whether DESI~DR2 confirms
$w_0\neq-1$ at $>3\sigma$; and whether $\vp^9\sqrt{\Ja\Jfour}=Q$ exactly
